...
function Enter-QQSkinOperationLock {
  $sid = [System.Security.Principal.WindowsIdentity]::GetCurrent().User.Value
  $mutex = [System.Threading.Mutex]::new($false, "Local\CodexQQSkin.$sid.Operation")
  $acquired = $false
  try {
  ...
  if (-not $acquired) {
    $mutex.Dispose()
    throw 'Another Codex Retro QQ Skin install, start, restore, or verify operation is already running.'
  }
  return $mutex
}

function Exit-QQSkinOperationLock {
  param([Parameter(Mandatory = $true)][System.Threading.Mutex]$Mutex)
  try { $Mutex.ReleaseMutex() } finally { $Mutex.Dispose() }
}

function Assert-QQSkinPort {
  param([Parameter(Mandatory = $true)][int]$Port)
  if ($Port -lt 1024 -or $Port -gt 65535) { throw "Port must be between 1024 and 65535: $Port" }
}

function Test-QQSkinPathEqual {
...
function Test-QQSkinPathWithin {
...
function Get-QQSkinRuntimeEnginePaths {
  ...
  return [pscustomobject]@{
    Root = $root
    Scripts = $scripts
    Start = Join-Path $scripts 'start-qq-skin.ps1'
    Restore = Join-Path $scripts 'restore-qq-skin.ps1'
    Tray = Join-Path $scripts 'tray-qq-skin.ps1'
  }
}

function Test-QQSkinTrayActive {
  $sid = [System.Security.Principal.WindowsIdentity]::GetCurrent().User.Value
...
function Assert-QQSkinRuntimeTree {
  param([Parameter(Mandatory = $true)][string]$Path)
  $root = [System.IO.Path]::GetFullPath($Path)
  if (-not (Test-Path -LiteralPath $root -PathType Container)) {
    throw "Retro QQ Skin runtime directory does not exist: $root"
  }
  if (-not (Get-Command Assert-QQSkinNoReparseComponents -ErrorAction SilentlyContinue)) {
    throw 'Retro QQ Skin managed-path validation is unavailable.'
  }
  Assert-QQSkinNoReparseComponents -Path $root
  foreach ($item in Get-ChildItem -LiteralPath $root -Recurse -Force -ErrorAction Stop) {
    if (($item.Attributes -band [System.IO.FileAttributes]::ReparsePoint) -ne 0) {
      throw "Retro QQ Skin runtime contains a junction or symbolic link: $($item.FullName)"
    }
  }
}
...
function Remove-QQSkinRuntimeTree {
  ...
  if (-not (Test-QQSkinPathWithin -Path $fullPath -Root $fullStateRoot)) {
    throw "Refusing to remove a runtime path outside the Retro QQ Skin state root: $fullPath"
  }
  if (-not (Test-Path -LiteralPath $fullPath)) { return }
  Assert-QQSkinRuntimeTree -Path $fullPath
  Remove-Item -LiteralPath $fullPath -Recurse -Force -ErrorAction Stop
}

function Install-QQSkinRuntimeEngine {
  param(
    [Parameter(Mandatory = $true)][string]$SkillRoot,
    [Parameter(Mandatory = $true)][string]$StateRoot
  )
  if (-not (Get-Command Ensure-QQSkinManagedDirectory -ErrorAction SilentlyContinue)) {
    throw 'Retro QQ Skin managed-directory validation is unavailable.'
  }

  $sourceRoot = [System.IO.Path]::GetFullPath($SkillRoot)
  $fullStateRoot = [System.IO.Path]::GetFullPath($StateRoot)
  $engine = Get-QQSkinRuntimeEnginePaths -StateRoot $fullStateRoot
  $required = @(
    ...
    'scripts\install-when-codex-exits.ps1',
    'scripts\install-qq-skin.ps1',
    'scripts\restore-qq-skin.ps1',
    'scripts\start-qq-skin.ps1',
    'scripts\theme-windows.ps1',
    'scripts\tray-qq-skin.ps1',
    'scripts\verify-qq-skin.ps1'
  )
  foreach ($relative in $required) {
    if (-not (Test-Path -LiteralPath (Join-Path $sourceRoot $relative) -PathType Leaf)) {
      throw "Retro QQ Skin runtime source is incomplete: $relative"
    }
  }
  foreach ($directoryName in @('assets', 'scripts')) {
    $sourceDirectory = Join-Path $sourceRoot $directoryName
    if ((Test-QQSkinPathEqual -Left $fullStateRoot -Right $sourceDirectory) -or
      (Test-QQSkinPathWithin -Path $fullStateRoot -Root $sourceDirectory)) {
      throw "Retro QQ Skin state root cannot be created inside its runtime source: $fullStateRoot"
    }
    Assert-QQSkinRuntimeTree -Path $sourceDirectory
  }

  Ensure-QQSkinManagedDirectory -Path $fullStateRoot -Root $fullStateRoot
  $token = [guid]::NewGuid().ToString('N')
  $stagingRoot = Join-Path $fullStateRoot ".engine-staging-$token"
  $backupRoot = Join-Path $fullStateRoot ".engine-backup-$token"
  Ensure-QQSkinManagedDirectory -Path $stagingRoot -Root $fullStateRoot
  ...
  try {
    ...
    Assert-QQSkinRuntimeTree -Path $stagingRoot
    foreach ($relative in $required) {
      if (-not (Test-Path -LiteralPath (Join-Path $stagingRoot $relative) -PathType Leaf)) {
        throw "Staged Retro QQ Skin runtime is incomplete: $relative"
      }
    }
    ...
    if ($sourceFiles.Count -ne $stagedFiles.Count) {
      throw 'Staged Retro QQ Skin runtime file count does not match its source.'
    }
    foreach ($sourceFile in $sourceFiles) {
      ...
      if (-not (Test-Path -LiteralPath $stagedFile -PathType Leaf) -or
        ...
        throw "Staged Retro QQ Skin runtime failed hash verification: $relative"
      }
    }
    ...
    if (Test-Path -LiteralPath $engine.Root) {
      Assert-QQSkinRuntimeTree -Path $engine.Root
    ...
    try {
      ...
      if ($hasBackup -and -not (Test-Path -LiteralPath $engine.Root)) {
        try {
          ...
          throw "Retro QQ Skin runtime update failed and its previous engine could not be restored. Backup preserved at ${backupRoot}: $installError"
        }
      }
      throw
    }
    if ($hasBackup) {
      try { Remove-QQSkinRuntimeTree -Path $backupRoot -StateRoot $fullStateRoot } catch {
        try {
    ...
    return Get-QQSkinRuntimeEnginePaths -StateRoot $fullStateRoot
  } finally {
    if (Test-Path -LiteralPath $stagingRoot) {
      try { Remove-QQSkinRuntimeTree -Path $stagingRoot -StateRoot $fullStateRoot } catch {
        try {
          Write-Warning "Could not remove the staged Retro QQ Skin runtime: $($_.Exception.Message)"
        } catch {
...
function Test-QQSkinCommandLineToken {
...
function ConvertTo-QQSkinProcessArgument {
...
function ConvertTo-QQSkinArgumentLine {
  param([AllowEmptyCollection()][string[]]$Arguments = @())
  return (($Arguments | ForEach-Object { ConvertTo-QQSkinProcessArgument -Value $_ }) -join ' ')
}

function Get-QQSkinProcessExecutablePath {
...
function Get-QQSkinNodeRuntime {
...
function ConvertTo-QQSkinCodexInstall {
  param(
...
function Get-QQSkinRegisteredCodexInstalls {
  $packages = @(Get-AppxPackage -Name 'OpenAI.Codex' -ErrorAction Stop | Sort-Object Version -Descending)
  $installs = @()
  foreach ($package in $packages) {
    $install = ConvertTo-QQSkinCodexInstall -Package $package
    if ($null -ne $install) { $installs += $install }
  }
  return $installs
}

function Get-QQSkinCodexInstall {
  $installs = @(Get-QQSkinRegisteredCodexInstalls)
...
function Initialize-QQSkinPackageLauncher {
  if ('CodexQQSkin.PackageLauncher' -as [type]) { return }
...
function Start-QQSkinCodex {
  ...
  $argumentLine = ConvertTo-QQSkinArgumentLine -Arguments $Arguments
  $processId = [CodexQQSkin.PackageLauncher]::Launch($appUserModelId, $argumentLine)
  if ($processId -le 0) { throw 'Windows did not return a Codex process ID after package activation.' }
  return $processId
}

function Get-QQSkinCodexStatePathCandidate {
  ...
  if (-not (Test-QQSkinPathEqual -Left $executable -Right $expectedExecutable)) { return $null }
  return [pscustomobject]@{
    PackageRoot = $packageRoot
    Executable = $executable
...
function Resolve-QQSkinCodexInstallFromState {
  param(
    [AllowNull()][object]$State,
    [Parameter(Mandatory = $true)][AllowEmptyCollection()][object[]]$RegisteredInstalls
  )
  $candidate = Get-QQSkinCodexStatePathCandidate -State $State
  ...
  foreach ($install in $RegisteredInstalls) {
    $pathMatches = (Test-QQSkinPathEqual -Left $candidate.PackageRoot -Right $install.PackageRoot) -and
      (Test-QQSkinPathEqual -Left $candidate.Executable -Right $install.Executable)
...
function Get-QQSkinCodexInstallFromState {
  param([AllowNull()][object]$State)
  try { $installs = @(Get-QQSkinRegisteredCodexInstalls) } catch { return $null }
  return Resolve-QQSkinCodexInstallFromState -State $State -RegisteredInstalls $installs
}

function Test-QQSkinWebSocketUrl {
...
function Test-QQSkinCdpPageTarget {
  ...
  if (-not (Test-QQSkinBrowserId -Value $targetId) -or
    -not (Test-QQSkinWebSocketUrl -Value $webSocketUrl -Port $Port)) {
    return $false
  }
  try {
...
function Get-QQSkinCdpTargets {
  param([int]$Port)
  try {
    $targets = Invoke-RestMethod -Uri "http://127.0.0.1:$Port/json/list" -TimeoutSec 2 `
      -MaximumRedirection 0 -ErrorAction Stop
    return @($targets | Where-Object { Test-QQSkinCdpPageTarget -Target $_ -Port $Port })
  } catch {
    return @()
  }
}

function Test-QQSkinBrowserId {
  param([string]$Value)
  return [bool]($Value -and $Value.Length -le 200 -and $Value -cmatch '^[A-Za-z0-9._-]+$')
}

function Get-QQSkinCdpBrowserIdentity {
  param([int]$Port)
  try {
    $version = Invoke-RestMethod -Uri "http://127.0.0.1:$Port/json/version" -TimeoutSec 2 `
      -MaximumRedirection 0 -ErrorAction Stop
    $webSocketUrl = "$($version.webSocketDebuggerUrl)"
    if (-not (Test-QQSkinWebSocketUrl -Value $webSocketUrl -Port $Port)) { return $null }
    ...
    if (-not (Test-QQSkinBrowserId -Value $browserId)) { return $null }
    return [pscustomobject]@{
      BrowserId = $browserId
...
function Get-QQSkinPortListeners {
  param([int]$Port)
...
function Test-QQSkinPortAvailable {
  param([int]$Port)
  return (Get-QQSkinPortListeners -Port $Port).Count -eq 0
}

function Test-QQSkinCodexPortOwner {
  param([int]$Port, [Parameter(Mandatory = $true)][object]$Codex)
  $listeners = Get-QQSkinPortListeners -Port $Port
  ...
  foreach ($listener in $listeners) {
    ...
    $processPath = if ($process) { Get-QQSkinProcessExecutablePath -ProcessInfo $process } else { $null }
    if (-not $processPath -or -not (Test-QQSkinPathEqual -Left $processPath -Right $Codex.Executable)) {
      return $false
    }
  }
  return $true
...
function Get-QQSkinVerifiedCdpIdentity {
  param([int]$Port, [Parameter(Mandatory = $true)][object]$Codex)
  if (-not (Test-QQSkinCodexPortOwner -Port $Port -Codex $Codex)) { return $null }
  $browser = Get-QQSkinCdpBrowserIdentity -Port $Port
  if ($null -eq $browser) { return $null }
  $targets = Get-QQSkinCdpTargets -Port $Port
...
function Test-QQSkinCodexCdpEndpoint {
  param([int]$Port, [Parameter(Mandatory = $true)][object]$Codex)
  return $null -ne (Get-QQSkinVerifiedCdpIdentity -Port $Port -Codex $Codex)
}

function Select-QQSkinPort {
  param([int]$PreferredPort)
  for ($candidate = $PreferredPort; $candidate -le [Math]::Min(65535, $PreferredPort + 100); $candidate++) {
    if (Test-QQSkinPortAvailable -Port $candidate) { return $candidate }
  }
  throw "No free loopback port was found between $PreferredPort and $([Math]::Min(65535, $PreferredPort + 100))."
}

function Wait-QQSkinPortAvailable {
  param([int]$Port, [int]$TimeoutSeconds = 5)
  $deadline = (Get-Date).AddSeconds($TimeoutSeconds)
  do {
    if (Test-QQSkinPortAvailable -Port $Port) { return $true }
    Start-Sleep -Milliseconds 200
  } while ((Get-Date) -lt $deadline)
  return $false
}

function Read-QQSkinState {
  param([Parameter(Mandatory = $true)][string]$Path)
  if (-not (Test-Path -LiteralPath $Path)) { return $null }
  try {
    $state = (Read-QQSkinUtf8File -Path $Path) | ConvertFrom-Json -ErrorAction Stop
    ...
    if ($properties -contains 'port') {
      ...
      Assert-QQSkinPort -Port $statePort
    }
    ...
    if ($properties -contains 'browserId' -and $state.browserId -and
      -not (Test-QQSkinBrowserId -Value "$($state.browserId)")) {
      throw 'State browser ID is invalid.'
    }
    return $state
  } catch {
    throw "Retro QQ Skin state is unreadable; it was preserved for inspection: $Path"
  }
}

function Write-QQSkinState {
  param([Parameter(Mandatory = $true)][string]$Path, [Parameter(Mandatory = $true)][object]$State)
  $json = $State | ConvertTo-Json -Depth 6
  Write-QQSkinUtf8FileAtomically -Path $Path -Content ($json + "`r`n")
}

function Archive-QQSkinStateFile {
  param([Parameter(Mandatory = $true)][string]$Path)
  if (-not (Test-Path -LiteralPath $Path)) { return $null }
...
function Get-QQSkinProcessStartedAt {
  param([int]$ProcessId)
  try {
...
function Stop-QQSkinRecordedInjector {
  param([AllowNull()][object]$State)
  ...
  $processPath = Get-QQSkinProcessExecutablePath -ProcessInfo $process
  ...
  $nodeMatches = -not $State.nodePath -or
    (Test-QQSkinPathEqual -Left $processPath -Right "$($State.nodePath)")
  $injectorMatches = [bool]($expectedInjector -and
    (Test-QQSkinCommandLineToken -CommandLine $commandLine -Token $expectedInjector) -and
    (Test-QQSkinCommandLineToken -CommandLine $commandLine -Token '--watch'))
  if ($State.port) {
  ...
  $startedAt = Get-QQSkinProcessStartedAt -ProcessId $processId
  ...
  if (-not $identityMatches) {
    throw "The recorded injector PID $processId is running, but its visible identity does not match the saved Retro QQ Skin process. State was preserved."
  }

  Stop-Process -Id $processId -Force -ErrorAction Stop
  try { Wait-Process -Id $processId -Timeout 5 -ErrorAction Stop } catch {}
  if (Get-Process -Id $processId -ErrorAction SilentlyContinue) {
    throw "The recorded Retro QQ Skin injector did not stop: PID $processId"
  }
  return $true
}

function Get-QQSkinCodexProcesses {
  ...
  return @(Get-CimInstance Win32_Process -Filter "Name = 'ChatGPT.exe'" -ErrorAction SilentlyContinue |
    Where-Object {
      $processPath = Get-QQSkinProcessExecutablePath -ProcessInfo $_
      Test-QQSkinPathEqual -Left $processPath -Right $Codex.Executable
    })
}

function Stop-QQSkinCodex {
  param([Parameter(Mandatory = $true)][object]$Codex, [switch]$AllowForce)
  $processes = Get-QQSkinCodexProcesses -Codex $Codex
  ...
  while ((Get-QQSkinCodexProcesses -Codex $Codex).Count -gt 0 -and (Get-Date) -lt $deadline) {
    Start-Sleep -Milliseconds 250
  }
  $remaining = Get-QQSkinCodexProcesses -Codex $Codex
  ...
  foreach ($item in $remaining) {
    ...
    $currentPath = if ($current) { Get-QQSkinProcessExecutablePath -ProcessInfo $current } else { $null }
    if ($currentPath -and (Test-QQSkinPathEqual -Left $currentPath -Right $Codex.Executable)) {
      Stop-Process -Id $item.ProcessId -Force -ErrorAction SilentlyContinue
    }
  }
  Start-Sleep -Milliseconds 500
  if ((Get-QQSkinCodexProcesses -Codex $Codex).Count -gt 0) { throw 'Codex could not be stopped safely.' }
}

function Confirm-QQSkinRestart {
  param([string]$Message)
  $shell = New-Object -ComObject WScript.Shell
  return $shell.Popup($Message, 0, 'Codex Retro QQ Skin', 52) -eq 6
}
